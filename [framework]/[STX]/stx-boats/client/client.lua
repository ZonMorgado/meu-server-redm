                    ...
                    if hour >= shopConfig.shopClose or hour < shopConfig.shopOpen then
                        ...
                        if (distanceShop <= shopConfig.distanceShop) and not IsPedInAnyBoat(player) then
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, CloseShops) then -- UiPromptHasStandardModeCompleted
                                ...
                                Wait(100)
                                lib.notify({ title = _U("shopClosed"), type = 'error', duration = 6000, position = 'center-right' })
                            end
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, OpenReturn) then -- UiPromptHasStandardModeCompleted
                                ...
                                if currentLocation == boatHome then
                                    ...
                                    if TransferAllow then
                                        ...
                                        TriggerServerEvent("stx-boats:TransferBoat", OwnedData, currentLocation, driveTransfer)
                                        ReturnBoat(shopId)
                                        lib.notify({ title = _U("your") .. boatName .. _U("available"), type = 'success', duration = 6000, position = 'center-right' })
                                    else
                                        ReturnBoat(shopId)
                                        lib.notify({ title = _U("your") .. boatName .. _U("returned") .. boatHome, type = 'success', duration = 6000, position = 'center-right' })
                                    end
                                end
                            end
                        end
...
function MainMenu(shopId)
    ...
    function(data, menu)
        ...
        if data.current.value == "own" then
            ...
            TriggerServerEvent('stx-boats:GetOwnedBoats', location, shopId)
        end
    end,
    function(data, menu)
        menu.close()
...
function BuyMenu(shopId)
    ...
    function(data, menu)
        ...
        if data.current.value then
            ...
            TriggerServerEvent('stx-boats:BuyBoat', buyData, location)
            menu.close()
            InMenu = false
...
RegisterNetEvent("stx-boats:OwnedBoatsMenu")
AddEventHandler("stx-boats:OwnedBoatsMenu", function(ownedBoats, shopId)
    MenuData.CloseAll()
    InMenu = true
    local elements = {}
...
function BoatMenu(shopId)
    ...
    function(data, menu)
        ...
        elseif data.current.value == "sell" then
            ...
            TriggerServerEvent('stx-boats:SellBoat', OwnedData, boatData)
            menu.close()
            InMenu = false
            ClearPedTasksImmediately(player)
            DisplayRadar(true)
        ...
        elseif data.current.value == "transfer" then
            ...
            if TransferAllow then
                ...
                lib.notify({ title = _U("transferDisabled"), type = 'error', duration = 6000, position = 'center-right' })
            end
        end
    end,
    function(data, menu)
    ...
    function(data, menu)
        ...
        if transferLocation then
            ...
            if transferLocation ~= location then
                TriggerServerEvent("stx-boats:TransferBoat", OwnedData, transferLocation, menuTransfer, boatData, shopName)
                menu.close()
                InMenu = false
                ClearPedTasksImmediately(PlayerPedId())
                DisplayRadar(true)
            else
                lib.notify({ title = _U("noTransfer"), type = 'error', duration = 6000, position = 'center-right' })
            end
        end
    end,
    function(data, menu)
        ...
        if data.current.value == "anchor" then
            if IsPedInAnyBoat(player) then
                ...
                if not isAnchored then
                    ...
                    isAnchored = true
                    lib.notify({ title = _U("anchorDown"), type = 'success', duration = 5000, position = 'center-right' })
                else
                    SetBoatAnchor(playerBoat, false)
                    isAnchored = false
                    lib.notify({ title = _U("anchorUp"), type = 'success', duration = 5000, position = 'center-right' })
                end
            end
            menu.close()
            InMenu = false
...
function SpawnBoat()
    ...
    IsBoating = true
    lib.notify({ title = _U("boatMenuTip"), type = 'success', duration = 6000, position = 'center-right' })
end
