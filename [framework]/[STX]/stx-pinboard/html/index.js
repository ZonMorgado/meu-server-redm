...
function loadTableData(table_img){
    ...
    for(var i in table_img){
        ...
        if (checkimage) {
            dataHtml += '<div class="box_'+i+'"><button type="button" onclick="open_image('+i+')" ><center>'+table_img[i].title+'</br><img src="'+table_img[i].poster_link+'"width="10vh" height="15vh"></button></center></div>'
        }else {
...
/* function send_values3() {
    btn.style.visibility = 'hidden';
    $.post('http://'+resourceName+'/client_getPoster', JSON.stringify({id:id}));
}
 */
