...
RegisterNetEvent('dfa-hunting:client:cooldownMessage')
AddEventHandler('dfa-hunting:client:cooldownMessage', function(remainingCooldown, totalCooldown)
...
RegisterNetEvent('dfa-hunting:server:useHuntingBait')
AddEventHandler('dfa-hunting:server:useHuntingBait', function(item)
    local playerServerId = GetPlayerServerId(PlayerId()) -- Get the player's server ID

    if inprogress then
        RSGCore.Functions.Notify('Already doing something', 'error')
        return
    end
        ...
        if currentZone then
            ...
            if item == requiredBait then -- Check if the used bait matches the required bait for the current zone
                ...
                if lib.progressCircle({
                    ...
                    --Wait(10000)
                    ClearPedTasks(ped)
                    TriggerServerEvent('dfa-hunting:server:removeItem', requiredBait)
    
                    -- Set the zoneCooldown for the current zone
                    remainingCooldowns[currentZone.name] = currentTime + currentZone.timer * 60
                    TriggerServerEvent('dfa-hunting:server:updateCooldown', currentZone.name, remainingCooldowns[currentZone.name])
