    ...
    if Config.PlayerRobery.Steal then
        ...
        while true do
            ...
            if isInMarker and not hasAlreadyEnteredMarker then
                ...
                TriggerEvent('stx-robplayer:inZone', currentZone)
            end

            if not isInMarker and hasAlreadyEnteredMarker then
                hasAlreadyEnteredMarker = false
                TriggerEvent('stx-robplayer:offZone', lastZone)
            end
        end
    end
end)

AddEventHandler('stx-robplayer:inZone', function(zone)
    currentZone = zone
end)

AddEventHandler('stx-robplayer:offZone', function(zone)
    if active == true then
        PromptSetEnabled(DeliverPrompt, false)
        PromptSetVisible(DeliverPrompt, false)
        active = false
    ...
    if Config.PlayerRobery.Steal then
        while true do
            ...
            if player ~= -1 and distance < Config.PlayerRobery.Distance then
                ...
                if isdead or cuffed or hogtied or lassoed or ragdoll or IsEntityPlayingAnim(playerPed, "script_proc@robberies@homestead@lonnies_shack@deception", "hands_up_loop", 3) then
                    if pressed then
                        TriggerServerEvent('stx-robplayer:RobPlayer')
                        pressed = false
                    end
                end
            end
