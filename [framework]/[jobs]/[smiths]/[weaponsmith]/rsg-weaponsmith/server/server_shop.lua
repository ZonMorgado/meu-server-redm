...
RSGCore.Functions.CreateCallback('rsg-weaponshop:server:weaponshopS', function(source, cb, currentweaponshop)
    MySQL.query('SELECT * FROM weapon_shop WHERE shopid = ?', {currentweaponshop}, function(result)
        if result[1] then
            cb(result)
        else
            cb(nil)
...
AddEventHandler('rsg-weaponshop:server:weaponshopPurchaseItem', function(location, item, amount)
    ...
    MySQL.query('SELECT * FROM weapon_stock WHERE shopid = ? AND items = ?',{location, item} , function(data)
        ...
        if price <= currentMoney then
            MySQL.update("UPDATE weapon_stock SET stock=@stock WHERE shopid=@location AND items=@item", {['@stock'] = stock, ['@location'] = location, ['@item'] = item}, function(count)
                if count > 0 then
                    ...
                    MySQL.query("SELECT * FROM weapon_shop WHERE shopid=@location", { ['@location'] = location }, function(data2)
                        local moneymarket = data2[1].money + price
                        MySQL.update('UPDATE weapon_shop SET money = ? WHERE shopid = ?',{moneymarket, location})
                    end)
...
RSGCore.Functions.CreateCallback('rsg-weaponshop:server:weaponshopGetMoney', function(source, cb, currentweaponshop)
    MySQL.query('SELECT * FROM weapon_shop WHERE shopid = ?', {currentweaponshop}, function(checkmoney)
...
AddEventHandler('rsg-weaponshop:server:weaponshopWithdraw', function(location, smoney)
    ...
    MySQL.query('SELECT * FROM weapon_shop WHERE shopid = ?',{location} , function(result)
        if result[1] ~= nil then
            if result[1].money >= tonumber(smoney) then
                local nmoney = result[1].money - smoney
                MySQL.update('UPDATE weapon_shop SET money = ? WHERE shopid = ?',{nmoney, location})
