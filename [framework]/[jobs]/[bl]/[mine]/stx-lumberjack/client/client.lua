    ...
    while true do
        ...
        if not IsPedOnMount(playerped) and not IsPedInAnyVehicle(playerped) and not IsPedDeadOrDying(playerped) then
            ...
            for k, v in pairs(Config.Trees) do
                local tree = DoesObjectOfTypeExistAtCoords(x, y, z, 1.55, GetHashKey(v), true)
                ...
                if tree and not InArray(ChoppedTrees, tostring(v)) then
                    ...
                    if PromptHasHoldModeCompleted(CuttingPrompt) then
                        ...
                        Citizen.Wait(500)
                        TriggerServerEvent("cools-lumberjack:axecheck", tostring(v))
                    end
                end
            end
        end
...
RegisterNetEvent("cools-lumberjack:axechecked")
AddEventHandler("cools-lumberjack:axechecked", function(tree)
    goChop(tree)
end)

RegisterNetEvent("cools-lumberjack:noaxe")
AddEventHandler("cools-lumberjack:noaxe", function()
    active = false
end)
...
function goChop(tree)
    ...
    while hastool == true do
        ...
        elseif IsControlJustReleased(0, Config.ChopTreeKey) then
            ...
            TriggerServerEvent('cools-lumberjack:addItem')
            Citizen.Wait(2000)  -- Removed GetExtraWait() function call
        end
